...
import { Bordure, Chief, ordinaries, Ordinary, Pale } from '../ordinary';
import {
  Charge,
  charges,
  CountAndDisposition,
...
const ordinaryArb: Arbitrary<Ordinary> = fc
  ...
  .chain(
    (obj): Arbitrary<Ordinary> => {
      ...
      if (obj.name === 'bordure' || obj.name === 'chief') {
        const name = obj.name;
        return lineArb.map((line): Bordure | Chief => ({ name, tincture: obj.tincture, line }));
      } else {
